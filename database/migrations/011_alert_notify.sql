...
RETURNS TRIGGER AS $$
BEGIN
    -- Bulk sources (GDELT) carry their own events and skip the LLM analyst; mission alerts
    -- skip the analyst too but must reach live listeners.
    IF COALESCE(NEW.metadata->>'skip_analysis', 'false') = 'true' THEN
        IF COALESCE(NEW.metadata->>'alert', 'false') <> 'true' THEN
    ...
    PERFORM pg_notify('new_intelligence', json_build_object(
        ...
        'alert', COALESCE(NEW.metadata->>'alert', 'false') = 'true',
