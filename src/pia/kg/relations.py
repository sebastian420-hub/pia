    ...
    with db.get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                ...
                FROM agg WHERE kind IS NOT NULL
            ...
            cur.execute("""
                ...
                ON CONFLICT (a_id, b_id, kind, source) DO UPDATE SET
                    ...
                    event_count = EXCLUDED.event_count, weight = EXCLUDED.weight, updated_at = NOW()
            """)
            # 3. decay / prune stale event relations that were not refreshed
            cur.execute("""
                DELETE FROM relations WHERE source IN ('events','cooccurrence') AND updated_at < NOW() - INTERVAL '2 hours'
            """)
        conn.commit()
